const XAL = artifacts.require('AltinLiraXAL.sol');
...
// Test that XAL operates correctly as an Ownable token.
contract('Ownable XAL', function ([_, admin, anotherAccount, anotherAccount2, owner]) {
  beforeEach(async function () {
    const xal = await XAL.new({from: owner});
  ...
  describe('as an initializable token', function () {
    ...
    it('constructor initializes the implementation contract and pauses it to avoid misleading state there', async function () {
      const isPaused = await this.xal.paused();
      assert.strictEqual(isPaused, true);
      const currentOwner = this.xal.owner();
      assert.notStrictEqual(currentOwner, ZERO_ADDRESS);
      await assertRevert(this.xal.initialize());
    });
  });
});
